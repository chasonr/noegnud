...
{
    printf("Usage: %s <referencefile> <tileset.png> <width> <height> "
           "[destination]\n",
           progname);
    exit(-1);
}
...
main(int argc, char **argv)
{
    ...
    slashem_image =
        calloc(nethack_image_surface->format->BytesPerPixel, width * height);
    ...
    if (argc > 5) {
        ...
        while (*generatename)
            generatename++;
        while (generatename >= argv[2] && *generatename != '/')
            generatename--;
        generatename++;
    ...
    for (y = 0; y < set_height; y++) {
        for (x = 0; x < set_width; x++) {
            ...
            if (setjmp(png_ptr->jmpbuf)) {
                fprintf(stderr,
                        "tile2png: Unknown problem while writing PNG.\n");
                fclose(fp);
                continue;
            }
            ...
            png_set_IHDR(png_ptr, info_ptr, width, height, 8,
                         nethack_image_surface->format->BytesPerPixel == 4
            ...
            for (row = 0; row < height; row++) {
                row_pointers[row] =
                    slashem_image
                    + row * width
                          * nethack_image_surface->format->BytesPerPixel;
            }

            for (dy = 0; dy < height; dy++) {
                pixels =
                    nethack_image_surface->pixels
                    + (((y * height) + dy) * nethack_image_surface->pitch)
                    + x * width
                          * nethack_image_surface->format->BytesPerPixel;
