...
void
add_to_enumeration(char *setting, int nparams, char *params, void *data)
{
...
void
gethelp(const char *progname)
{
    printf("Usage: %s <referencefile> <tilesetdir> <columns> "
           "<joinedimagename.png> <tile_width> <tile_height> <mid_x> <mid_y> "
           "[imagemap.html]\n",
           progname);
...
main(int argc, char **argv)
{
    ...
    noegnud_typeloader_parsefile(add_to_enumeration, argv[1],
                                 &tileset_enumerate);
    ...
    if (argc == 3 && (!strcmp("test", argv[2]))) {
        ...
        for (set_width = sqrt(desttilesetsize); set_width > 0; set_width--) {
            ...
            printf("resizing %4d tiles into ( %4d x %4d ) blocks, %4d "
                   "placements, %4d loss",
                   desttilesetsize, set_width, set_height,
                   set_width * set_height,
                   set_width * set_height - desttilesetsize);
    ...
    printf("resizing %4d tiles into ( %4d x %4d ) blocks, %4d placements, "
           "%4d loss\n",
           desttilesetsize, set_width, set_height, set_width * set_height,
           set_width * set_height - desttilesetsize);
    ...
    */
    ...
    slashem_image =
        calloc(output_bpp, (width * height) * (set_width * set_height));

    if (argc > 9) {
        web = fopen(argv[9], "w");
        fprintf(web, "<HTML>\n\t<BODY "
                     "background=\"imagemap_back.png\">\n\t\t<CENTER>"
                     "\n\t\t\t<MAP name=\"noegnud\">\n");
    }
    ...
    for (y = 0; y < set_height; y++) {
        for (x = 0; x < set_width; x++) {
            if (y * set_width + x >= desttilesetsize)
                continue;
            ...
            if (argc > 9) {
                fprintf(
                    ...
                    "%s\"\n\t\t\t\t\tcoords=\"%d,%d,%d,%"
                    "d\"\n\t\t\t\t\tnohref\n\t\t\t\t\tonMouseOver=\"window."
                    "status='%d - %s';return true;\">\n",
                    y * set_width + x, fullname, x * width, y * height,
                    x * width + width - 1, y * height + height - 1,
                    y * set_width + x, fullname);
            }
            ...
            if (!nethack_image_surface) {
                printf("* invalid image file, or error loading image (%s) "
                       "for tile number %d.\n",
                       fullpath, y * set_width + x);
                continue;
            }

            if (nethack_image_surface->w > largest_x)
                largest_x = nethack_image_surface->w;
            if (nethack_image_surface->h > largest_y)
                largest_y = nethack_image_surface->h;
            ...
            for (dy = 0; dy < nethack_image_surface->h; dy++) {
                pixels = nethack_image_surface->pixels
                         + (dy * nethack_image_surface->pitch);

                for (dx = 0; dx < nethack_image_surface->w; dx++) {
                    SDL_GetRGBA(*((Uint32 *) pixels),
                                nethack_image_surface->format, &r, &g, &b,
                                &a);

                    rgba = slashem_image
                           + (((y * height + (dy + mid_y_use))
                               * (width * set_width))
                              + (x * width) + dx
                              - (nethack_image_surface->w / 2) + (mid_x))
                                 * output_bpp;
    ...
    row_pointers =
        (png_bytep *) malloc(height * set_height * sizeof(png_bytep));
    for (row = 0; row < height * set_height; row++) {
        row_pointers[row] =
            slashem_image + row * (set_width * width * output_bpp);
    }
    ...
    if (setjmp(png_ptr->jmpbuf)) {
        ...
        png_set_IHDR(
            png_ptr, info_ptr, set_width * width, set_height * height, 8,
