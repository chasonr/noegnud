...
noegnud_glfuncs_maximum_texturesize_init(void)
{
    ...
    for (noegnud_glfuncs_maximum_texturesize =
         ...
         noegnud_glfuncs_maximum_texturesize > 64;
         noegnud_glfuncs_maximum_texturesize /= 2) {
        glGenTextures(1, &texture);
        glBindTexture(GL_TEXTURE_2D, texture);

        glTexImage2D(GL_TEXTURE_2D, 0, 4, noegnud_glfuncs_maximum_texturesize,
                     noegnud_glfuncs_maximum_texturesize, 0, GL_RGBA,
...
noegnud_glfuncs_loadimage(const char *fname, int split, int tileable,
                          int forcemipmaps)
{
    ...
    if (!noegnud_common_file_exists(filename))
        sprintf(filename, "../" NOEGNUD_DATADIR "/%s", fname);
    ...
    if ((tileable || (!split))
        || (image->w <= noegnud_glfuncs_maximum_texturesize
            && image->h <= noegnud_glfuncs_maximum_texturesize)) {
        glimage->block_width = 1;
        glimage->block_height = 1;
    } else {
        glimage->block_width =
            (image->w + (noegnud_glfuncs_maximum_texturesize - 1))
            / noegnud_glfuncs_maximum_texturesize;
        glimage->block_height =
            (image->h + (noegnud_glfuncs_maximum_texturesize - 1))
            / noegnud_glfuncs_maximum_texturesize;
    }

    for (block_y = 0; block_y < glimage->block_height; block_y++)
        for (block_x = 0; block_x < glimage->block_width; block_x++) {
            glimage->block[block_x][block_y] =
                noegnud_mem_malloc(sizeof(noegnud_glfuncs_timageblock));
            ...
            if ((tileable || (!split))
                ...
                glimage->block[block_x][block_y]->width =
                    noegnud_glfuncs_maximum_texturesize;
                if ((image->w - block_x * noegnud_glfuncs_maximum_texturesize)
                    < glimage->block[block_x][block_y]->width)
                    glimage->block[block_x][block_y]->width =
                        (image->w
                         - block_x * noegnud_glfuncs_maximum_texturesize);
                glimage->block[block_x][block_y]->height =
                    noegnud_glfuncs_maximum_texturesize;
                if ((image->h - block_y * noegnud_glfuncs_maximum_texturesize)
                    < glimage->block[block_x][block_y]->height)
                    glimage->block[block_x][block_y]->height =
                        (image->h
                         - block_y * noegnud_glfuncs_maximum_texturesize);
            }

            req_width = noegnud_common_power2(
                glimage->block[block_x][block_y]->width);
            req_height = noegnud_common_power2(
                glimage->block[block_x][block_y]->height);

            glimage->block[block_x][block_y]->glwidth = req_width;
            glimage->block[block_x][block_y]->glheight = req_height;

            glimage->block[block_x][block_y]->fx1 =
                glimage->block[block_x][block_y]->fy1 = 0.0;
            glimage->block[block_x][block_y]->fx2 =
                (float) glimage->block[block_x][block_y]->width
                / (float) req_width;
            glimage->block[block_x][block_y]->fy2 =
                (float) glimage->block[block_x][block_y]->height
                / (float) req_height;

            tmp = noegnud_mem_sdl_creatergbsurface(
                SDL_SWSURFACE, req_width, req_height,
            ...
            glBindTexture(GL_TEXTURE_2D,
                          glimage->block[block_x][block_y]->image);
            ...
            if (tmp->format->BytesPerPixel == 4) {
                if ((noegnud_options_render_opengl_mipmapping->value)
                    && (tileable || (!split) || forcemipmaps)) {
                    gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA, req_width,
                                      req_height, GL_RGBA, GL_UNSIGNED_BYTE,
                                      tmp->pixels);
                } else {
                    glTexImage2D(GL_TEXTURE_2D, 0, 4, req_width, req_height,
                                 0, GL_RGBA, GL_UNSIGNED_BYTE, tmp->pixels);
                }
            } else {
                if ((noegnud_options_render_opengl_mipmapping->value)
                    && (tileable || (!split) || forcemipmaps)) {
                    gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, req_width,
                                      req_height, GL_RGB, GL_UNSIGNED_BYTE,
                                      tmp->pixels);
                } else {
                    glTexImage2D(GL_TEXTURE_2D, 0, 3, req_width, req_height,
                                 0, GL_RGB, GL_UNSIGNED_BYTE, tmp->pixels);
                }
            }
...
void
noegnud_glfuncs_unloadimage(noegnud_glfuncs_timage *image)
{
    int block_x, block_y;
