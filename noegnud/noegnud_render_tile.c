...
noegnud_render_map_single_tile(int glyph_tile, int normalface)
{
    ...
    glVertex3f(noegnud_activetileset->glwidth, 0,
...
noegnud_render_map_single_tile_raised_walls_texture(GLint texture,
                                                    ...
                                                    int b, int l, int r)
{
    if (noegnud_options_render_opengl_lighting_method->value
    ...
    if (a) {
        ...
        glTexCoord2f(noegnud_activetileset->fx1, noegnud_activetileset->fy1);
        glVertex3f(noegnud_activetileset->glwidth,
                   noegnud_activetileset->glheight * height, 0);
    ...
    if (b) {
        ...
        glTexCoord2f(noegnud_activetileset->fx1, noegnud_activetileset->fy1);
        glVertex3f(noegnud_activetileset->glwidth,
                   noegnud_activetileset->glheight * height,
                   noegnud_activetileset->glheight);

        glTexCoord2f(noegnud_activetileset->fx2, noegnud_activetileset->fy1);
        glVertex3f(0, noegnud_activetileset->glheight * height,
                   noegnud_activetileset->glheight);
    }
...
noegnud_render_map_tile_highlight_block()
{
    ...
    glColor4f(1.0, 1.0, 1.0,
              noegnud_options_render_highlight_block_opacity->value);

    glBegin(GL_QUADS);

    glVertex3f(0, 0, noegnud_activetileset->glheight);
    glVertex3f(noegnud_activetileset->glwidth / 10, 0,
               noegnud_activetileset->glheight);
    glVertex3f(noegnud_activetileset->glwidth / 10, 0, 0);
    glVertex3f(0, 0, 0);

    glVertex3f(noegnud_activetileset->glwidth * 9 / 10, 0,
               noegnud_activetileset->glheight);
    glVertex3f(noegnud_activetileset->glwidth, 0,
               noegnud_activetileset->glheight);
    glVertex3f(noegnud_activetileset->glwidth, 0, 0);
    ...
    glVertex3f(noegnud_activetileset->glwidth / 10, 0,
               noegnud_activetileset->glheight / 10);
    glVertex3f(noegnud_activetileset->glwidth * 9 / 10, 0,
               noegnud_activetileset->glheight / 10);
    glVertex3f(noegnud_activetileset->glwidth * 9 / 10, 0, 0);
    glVertex3f(noegnud_activetileset->glwidth / 10, 0, 0);
    ...
    glVertex3f(noegnud_activetileset->glwidth * 9 / 10, 0,
               noegnud_activetileset->glheight);
    glVertex3f(noegnud_activetileset->glwidth * 9 / 10, 0,
               noegnud_activetileset->glheight * 9 / 10);
    glVertex3f(noegnud_activetileset->glwidth / 10, 0,
               noegnud_activetileset->glheight * 9 / 10);
...
noegnud_render_map_tile()
{
    ...
    realangle =
        (noegnud_options_render_opengl_view_angle_x_current->value
         + (noegnud_activetileset->iso ? 0
                                       : noegnud_render_bullettime_angle));
    while (realangle >= 360)
        realangle -= 360;
    while (realangle < 0)
        realangle += 360;
    ...
    if (noegnud_activetileset->iso) {
        ...
        if (noegnud_options_render_features_cardboardcutout->value
            && noegnud_activetileset->isalphatileset) {
    ...
    for (y = 0; y < MAX_MAP_Y; y++) {
        ...
        for (x = 0; x < MAX_MAP_X; x++) {
            ...
            /*
            if
            (noegnud_options_input_mouse_3d_light->value&&((noegnud_render_mouse_map_x-1)==x)&&(noegnud_render_mouse_map_y==y))
            { glLightfv(GL_LIGHT1,GL_POSITION,light_position);
            }
            */

            if (((noegnud_render_mouse_map_x - 1) == x)
                && (noegnud_render_mouse_map_y == y)) {
                glColor4f(0.25, 1.0, 0.25, 1.0);
            } else if ((x == noegnud_clip_x) && (y == noegnud_clip_y)) {
                glColor4f(noegnud_render_highlight_strobe / 2 + 0.5,
                          noegnud_render_highlight_strobe / 2 + 0.5,
                          noegnud_render_highlight_strobe / 2 + 0.5, 1.0);
            ...
            if (noegnud_render_selection_busy) {
                noegnud_render_map_single_selection_tile(
                    noegnud_map[x][y].tile_primary, 1);
            } else if (noegnud_options_mode_tileset_caverns->value
                ...
                if (levl[x + 1][y].seenv) {
                    if (noegnud_options_render_features_cardboardcutout->value
                        ...
                        noegnud_render_map_single_tile_raised_walls_texture(
                            ...
                            !((y < MAX_MAP_Y - 1)
                              && noegnud_map[x][y + 1].high),
                            !((x > 0) && noegnud_map[x - 1][y].high),
                            !((x < MAX_MAP_X - 1)
                              && noegnud_map[x + 1][y].high));
                ...
                if (levl[x + 1][y].seenv) {
                    if (IS_ROOM(levl[x + 1][y].typ)) {
                        ...
                        glNormal3f(0, 1, 0);
                        glTexCoord2f(noegnud_image_dungeon->block[0][0]->fx1,
                                     noegnud_image_dungeon->block[0][0]->fy2);
                        glVertex3f(0, 0, noegnud_activetileset->glheight);

                        glTexCoord2f(noegnud_image_dungeon->block[0][0]->fx2,
                                     noegnud_image_dungeon->block[0][0]->fy2);
                        glVertex3f(noegnud_activetileset->glwidth, 0,
                                   noegnud_activetileset->glheight);

                        glTexCoord2f(noegnud_image_dungeon->block[0][0]->fx2,
                                     noegnud_image_dungeon->block[0][0]->fy1);
                        glVertex3f(noegnud_activetileset->glwidth, 0, 0);
                    ...
                    if (noegnud_options_mode_tileset_caverns_unexplored
                            ->value) {
                        glBindTexture(
                            GL_TEXTURE_2D,
                            noegnud_image_dungeon->block[0][0]->image);
            ...
            if (((!noegnud_activetileset->isalphatileset)
                 && (noegnud_map[x][y].glyph_primary != MAX_GLYPH))) {
                noegnud_render_map_single_tile(noegnud_map[x][y].tile_primary,
                                               1);
                if ((((x == noegnud_clip_x) && (y == noegnud_clip_y))
                     || (((noegnud_render_mouse_map_x - 1) == x)
                         && (noegnud_render_mouse_map_y == y)))
                    ||
                    //				if (((x == noegnud_clip_x) && (y ==
                    //noegnud_clip_y)) ||
                    (noegnud_map[x][y].glyph_primary != MAX_GLYPH
                     && iflags.hilite_pet
                     && (noegnud_map[x][y].special_primary & MG_PET))) {
                ...
                if (noegnud_map[x][y].glyph_back != MAX_GLYPH
                    ...
                    noegnud_render_map_single_tile(
                        noegnud_map[x][y].tile_back, 1);
                if ((!noegnud_render_selection_busy)
        ...
        for (y = 0; y < MAX_MAP_Y; y++) {
            ...
            for (x = 0; x < MAX_MAP_X; x++) {
				...
				(sqrt((noegnud_clip_y-y)*(noegnud_clip_y-y)+(noegnud_clip_x-x)*(noegnud_clip_x-x))<3)
                    ...
                    current_alpha =
                        noegnud_options_mode_tileset_iso_opacity->value;
                } else {
                    current_alpha = 1.0;
                }
                ...
                if (noegnud_image_dungeon
                    && (noegnud_map[x][y].glyph_primary == MAX_GLYPH
                        || (glyph_to_cmap(noegnud_map[x][y].glyph_primary)
                                == S_stone
                            && levl[x + 1][y].lit))) {
                    if (levl[x + 1][y].seenv) {
                        if (noegnud_options_render_features_cardboardcutout
                                ->value
                            && noegnud_options_mode_tileset_caverns_cutout
                                   ->value
                            && noegnud_options_render_features_cardboardcutout_raise_walls
                ...
                if (noegnud_activetileset->isalphatileset) {
                    if (noegnud_map[x][y].glyph_middle != MAX_GLYPH) {
                        noegnud_render_map_single_tile(
                            noegnud_map[x][y].tile_middle, 1);
                    }
                    if (noegnud_activetileset->iso
                        && noegnud_map[x][y].glyph_fore != MAX_GLYPH) {
                        noegnud_render_map_single_tile(
                            noegnud_map[x][y].tile_fore, 1);
                    }
                    if (((x == noegnud_clip_x) && (y == noegnud_clip_y))
                        || (((noegnud_render_mouse_map_x - 1) == x)
                            && (noegnud_render_mouse_map_y == y))
                        || (noegnud_map[x][y].glyph_primary != MAX_GLYPH
                            && iflags.hilite_pet
                            && (noegnud_map[x][y].special_primary
                                & MG_PET))) {
                        if (noegnud_options_render_features_cardboardcutout
                                ->value)
    ...
    if ((noegnud_activetileset->isalphatileset)
            ...
            for (y = ys; y != ye; y += yd) {
                ...
                for (x = xs; x != xe; x += xd) {
                    ...
                    if (!(noegnud_map[x][y].high || noegnud_map[x][y].low)) {
                        if (noegnud_map[x][y].glyph_back != MAX_GLYPH) {
                            if (noegnud_options_render_opengl_lighting_method
                                    ->value
                                == NOEGNUD_RENDER_LIGHTING_METHOD_CHEAP) {
                                glEnable(GL_LIGHTING);
                            }
                    ...
                    if (noegnud_map[x][y].glyph_fore != MAX_GLYPH) {
                        if ((x == (u.ux - 1) && y == u.uy) && Levitation) {
                            ...
                            glTranslatef(
                                0, noegnud_activetileset->glheight / 2, 0);
                        }
                        if (noegnud_map[x][y].low)
                            glTranslatef(
                                0,
                                -noegnud_activetileset->glheight
                                    * noegnud_options_render_features_cardboardcutout_hole_depth
                                          ->value,
                                0);
                        if (((glyph_is_monster(noegnud_map[x][y].glyph_fore)
                              && noegnud_options_render_features_cardboardcutout_raise_monsters
                                     ->value)
                             || (glyph_is_object(noegnud_map[x][y].glyph_fore)
                                 && noegnud_options_render_features_cardboardcutout_raise_objects
                                        ->value))) {
                            glTranslatef(0, noegnud_activetileset->glheight,
                                         noegnud_activetileset->glheight / 2);
                            glRotatef(90, 1, 0, 0);
                            if (noegnud_options_render_features_autoface
                                    ->value) {
                                glTranslatef(
                                    noegnud_activetileset->glwidth / 2, 0, 0);
                                glRotatef(
                                    (noegnud_options_render_opengl_view_angle_x_current
                                         ->value
                                     + noegnud_render_bullettime_angle),
                                    0, 0, 1);
                                glTranslatef(-noegnud_activetileset->glwidth
                                                 / 2,
                                             0, 0);
                            }
                            glRotatef(-20, 1, 0, 0);
        ...
        for (y = ys; y != ye; y += yd) {
            ...
            for (x = xs; x != xe; x += xd) {
                ...
                if ((noegnud_activetileset->iso) && (y >= noegnud_clip_y)
                    && (!((x == noegnud_clip_x) && (y == noegnud_clip_y)))
                    && (!glyph_is_object(noegnud_map[x][y].glyph_primary))
                    && (!glyph_is_monster(noegnud_map[x][y].glyph_primary))
                    && (sqrt((noegnud_clip_y - y) * (noegnud_clip_y - y)
                             + (noegnud_clip_x - x) * (noegnud_clip_x - x))
                        < 3)) {
                    current_alpha = 0.3;
                } else {
                    current_alpha = 1.0;
                ...
                if (noegnud_map[x][y].glyph_fore != MAX_GLYPH) {
                    if ((x == (u.ux - 1) && y == u.uy)
                        && (!noegnud_activetileset->iso) && Levitation) {
                        if (noegnud_map[x][y].low) {
                            glTranslatef(
                                0,
                                noegnud_activetileset->glheight
                        ...
                        glTranslatef(0, noegnud_activetileset->glheight / 2,
                                     0);
                    }
                    if (noegnud_map[x][y].low
                        && (!noegnud_activetileset->iso))
                    ...
                    if (noegnud_options_render_features_cardboardcutout->value
                        ...
                        if (noegnud_options_render_features_autoface->value) {
                            glTranslatef(noegnud_activetileset->glwidth / 2,
                                         0, 0);
                            glRotatef(
                                (noegnud_options_render_opengl_view_angle_x_current
                                     ->value
