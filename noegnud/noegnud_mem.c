...
void
noegnud_mem_debug_printused(char *filename, int linenumber)
{
    printf(" [%s] (%d) : %d\n", filename, linenumber,
...
tnoegnud_mem_block *
noegnud_mem_memblock_create(const char *sourcefile, const int linenumber,
...
void *
noegnud_mem_debug_malloc(const char *filename, const int linenumber, int size)
{
    void *ptr = 0;
    char buf[256];
    ...
    if (noegnud_debug_mem_memused > noegnud_debug_mem_mostmemused)
        noegnud_debug_mem_mostmemused = noegnud_debug_mem_memused;

    sprintf(buf, "malloc_0x%X", (int) ptr);

    memblock = noegnud_mem_memblock_create(filename, linenumber, ptr, size);
    memstruct =
        noegnud_collection_structure(noegnud_debug_mem_ptrsizemap, buf);
    if (memstruct) {
...
void
noegnud_mem_debug_free(const char *filename, const int linenumber, void *ptr)
{
    ...
    while (ptrsize_find && (!ptrsize)) {
        ...
        if ((!strcmp(ptrsize_find->name, buf)) && memblock
            && (memblock->size))
            ptrsize = ptrsize_find;
        ptrsize_find = ptrsize_find->next;
    }
    // ptrsize=noegnud_collection_structure(noegnud_debug_mem_ptrsizemap,buf);
    if (!ptrsize) {
        printf("[DEBUG:%s:%d] *WARNING* failed to find debug reference for "
               "pointer (\"%s\") !\n",
...
#define noegnud_mem_debug_getsize_sdlsurface(_surface) \
    (sizeof(SDL_Surface)                               \
     + _surface->w * _surface->h * _surface->format->BytesPerPixel)

SDL_Surface *
noegnud_mem_debug_img_load(const char *filename, const int linenumber,
                           const char *image_filename)
{
    ...
    if (tmp) {
        ...
        memblock = noegnud_mem_memblock_create(
            filename, linenumber, tmp,
            noegnud_mem_debug_getsize_sdlsurface(tmp));
        noegnud_debug_mem_memused +=
            noegnud_mem_debug_getsize_sdlsurface(tmp);
        memstruct =
            noegnud_collection_structure(noegnud_debug_mem_ptrsizemap, buf);
...
SDL_Surface *
noegnud_mem_debug_sdl_creatergbsurface(const char *filename,
                                       const int linenumber, Uint32 flags,
                                       ...
                                       Uint32 Bmask, Uint32 Amask)
{
    ...
    tmp = SDL_CreateRGBSurface(flags, width, height, depth, Rmask, Gmask,
                               Bmask, Amask);
    if (tmp) {
        sprintf(buf, "sdl_surface_0x%X", (int) tmp);
        memblock = noegnud_mem_memblock_create(
...
SDL_Surface *
noegnud_mem_debug_sdl_creatergbsurfacefrom(const char *filename,
                                           const int linenumber, void *pixels,
                                           Uint32 flags, int width,
                                           ...
                                           Uint32 Bmask, Uint32 Amask)
{
    ...
    tmp = SDL_CreateRGBSurfaceFrom(pixels, flags, width, height, depth, Rmask,
                                   Gmask, Bmask, Amask);
    if (tmp) {
        sprintf(buf, "sdl_surface_0x%X", (int) tmp);
        memblock = noegnud_mem_memblock_create(
...
void
noegnud_mem_debug_sdl_freesurface(const char *filename, const int linenumber,
                                  SDL_Surface *surface)
{
    noegnud_tcollection *ptrsize;
    char buf[256];
