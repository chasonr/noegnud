...
noegnud_render_map_single_char(int x, int y, int glyph_char, int colour)
{
    ...
    // I dont know yet why i need the following line :/ needs fixing @ some
    // stage ....
    if (colour == CLR_BLACK)
        colour = CLR_BLUE;
    ...
    glColor4f(noegnud_nethackcolours[colour][0] * highlight_multiplier,
    ...
    glVertex3f(noegnud_font_map->glwidth * NOEGNUD_CHAR_MODIFIER, 0,
...
noegnud_render_map_char_highlight_block()
{
    ...
    /*
            ...
            glVertex3f (noegnud_font_map->glwidth, 0,
       noegnud_font_map->glheight); glVertex3f (noegnud_font_map->glwidth, 0,
       0); glVertex3f (0, 0, 0);
    */
...
noegnud_render_map_char()
{
    ...
    realangle = (noegnud_options_render_opengl_view_angle_x_current->value
    ...
    if ((noegnud_options_render_features_cardboardcutout->value)
        ...
        for (y = 0; y < MAX_MAP_Y; y++) {
            for (x = 0; x < MAX_MAP_X; x++) {
                ...
                glPushMatrix();
                glTranslatef(
                    (noegnud_font_map->glwidth * NOEGNUD_CHAR_MODIFIER) * (nx)
                ...
                } else if (noegnud_map[x][y].glyph_middle != MAX_GLYPH) {
                    if (iflags.wc_color) {
                        noegnud_render_map_single_char(
                            x, y, noegnud_map[x][y].ch_middle,
                            noegnud_map[x][y].colour_middle);
                    } else {
                        noegnud_render_map_single_char(
                            x, y, noegnud_map[x][y].ch_middle, CLR_WHITE);
                    }
                } else if (noegnud_map[x][y].glyph_back != MAX_GLYPH) {
                    if (iflags.wc_color) {
                        noegnud_render_map_single_char(
                            x, y, noegnud_map[x][y].ch_back, CLR_GRAY);
                    } else {
                        noegnud_render_map_single_char(
                            x, y, noegnud_map[x][y].ch_back, CLR_WHITE);
                    }
                }
        ...
        for (x = 0; x < MAX_MAP_X; x++) {
            ...
            glTranslatef(
                (noegnud_font_map->glwidth * NOEGNUD_CHAR_MODIFIER) * (nx)
                    + noegnud_render_offset_x,
                0,
                -(noegnud_font_map->glheight * NOEGNUD_CHAR_MODIFIER) * (ny)
                    + noegnud_render_offset_y);
            if (noegnud_options_render_features_cardboardcutout->value) {
                if (noegnud_map[x][y].glyph_fore != MAX_GLYPH) {
                    glTranslatef(
                        0, noegnud_font_map->glheight * NOEGNUD_CHAR_MODIFIER,
                        noegnud_font_map->glheight * NOEGNUD_CHAR_MODIFIER
                            / 2);
                    glRotatef(70, 1, 0, 0);
                    if (iflags.wc_color) {
                        noegnud_render_map_single_char(
                            x, y, noegnud_map[x][y].ch_fore,
                            noegnud_map[x][y].colour_fore);
                    } else {
                        noegnud_render_map_single_char(
                            x, y, noegnud_map[x][y].ch_fore, CLR_WHITE);
                    }
                }
            } else if (noegnud_map[x][y].glyph_primary != MAX_GLYPH) {
                if (iflags.wc_color) {
                    noegnud_render_map_single_char(
                        x, y, noegnud_map[x][y].ch_primary,
                        noegnud_map[x][y].colour_primary);
                } else {
                    noegnud_render_map_single_char(
                        x, y, noegnud_map[x][y].ch_primary, CLR_WHITE);
                }
