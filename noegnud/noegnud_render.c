...
noegnud_render_loop(void)
{
    ...
    glLoadIdentity();
    glOrtho(0, noegnud_options_screenwidth->value, 0,
            noegnud_options_screenheight->value, -1, 1);
    ...
    if (sdltimer - noegnud_render_bullettime_lastevent
        > noegnud_options_render_bullettime_trigger->value) {
        noegnud_render_bullettime_angle =
            ((float) (sdltimer
                      - (noegnud_render_bullettime_lastevent
                         + noegnud_options_render_bullettime_trigger->value)))
            / 50;
    }

    noegnud_render_highlight_strobe =
        (sin(((((float) sdltimer) / 5.0)
              * noegnud_options_render_highlight_strobe_speed->value)
             * 3.1415926535897931 / 180.0)
         + 1.0)
        / 2.0;

    if ((fpslimiter_diff = (sdltimer - noegnud_render_lastframe_time))
        < (fpslimiter_min =
               (1000 / noegnud_options_render_engine_fps_limit->value)))
    ...
    if (noegnud_options_render_engine_fps_show->value
        ...
        sprintf(fps_string, "%4.1f fps",
                1000.0
                    / ((float) ((sdltimer - noegnud_render_lastframe_time))
                       + 0.001));
...
noegnud_render_map()
{
    ...
    if (noegnud_render_selection_busy) {
        ...
        gluPickMatrix(noegnud_render_selection_x,
                      (viewport_coords[3] - noegnud_render_selection_y), 1, 1,
                      viewport_coords);
    }
    ...
    gluPerspective(noegnud_options_render_opengl_view_fov->value,
                   ...
                   noegnud_options_render_opengl_view_min->value,
                   noegnud_options_render_opengl_view_max->value);
    ...
    if ((noegnud_options_mode_drawmode->value == NOEGNUD_RENDER_DRAWMODE_TILE)
        && (noegnud_activetileset && noegnud_activetileset->iso)) {
        glRotatef(90, 1, 0, 0);
        glRotatef(noegnud_options_render_opengl_view_angle_x_current->value,
                  0, 1, 0);
    } else {
        glRotatef(noegnud_options_render_opengl_view_angle_y_current->value,
                  1, 0, 0);
        glRotatef(noegnud_options_render_opengl_view_angle_x_current->value
                      + noegnud_render_bullettime_angle,
                  0, 1, 0);
...
void
noegnud_render_selection(int x, int y)
{
    int tiles_selected = 0;

    if ((!noegnud_options_input_3dmouse->value) || noegnud_gui_mouseoverwidget
        || noegnud_gui_mouseoverminimap) {
        if (!noegnud_gui_mouseoverminimap)
            noegnud_render_mouse_map_x = -1;
        return;
    }
    ...
    if (tiles_selected) {
        ...
        for (i = 1; i < tiles_selected; i++) {
            if (noegnud_render_selection_buffer[(i * 4) + 1] < lowest_depth) {
                ...
                selected_object =
                    noegnud_render_selection_buffer[(i * 4) + 3];
            }
        }
