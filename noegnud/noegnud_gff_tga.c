...
static void noegnud_gff_tga_header(FILE *f, int width, int height, int bits);

static void
noegnud_gff_tga_header(FILE *f, int width, int height, int bits)
{
    fputc(0, f);
    fputc(0, f);
    fputc(2, f);
...
int
noegnud_gff_tga_save(const char *filename, int width, int height,
                     void *rgbdata)
{
    FILE *f;
    int c;
