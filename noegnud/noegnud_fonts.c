...
noegnud_fonts_loadimage(noegnud_fonts_tfont *font)
{
    ...
    font->glwidth =
        ((float) font->width / 32) * noegnud_options_render_multiplier->value;
    font->glheight = ((float) font->height / 32)
    ...
    tmp = noegnud_mem_sdl_creatergbsurface(
        SDL_SWSURFACE, req_width, req_height, image->format->BitsPerPixel,
        ...
        for (x = 0; x < 16; x++) {
            ...
            for (dx = 0; dx < font->width; dx++) {
                for (dy = 0; dy < font->height; dy++) {
                    pixels = tmp->pixels + (dy * tmp->pitch)
                             + dx * tmp->format->BytesPerPixel;
                    SDL_GetRGBA(*((Uint32 *) pixels), tmp->format, &r, &g, &b,
                                &a);
                    if (a == 255) {
            ...
            for (dx = font->width - 1; dx > 0; dx--) {
                ...
                if (font->dynamic[y * 16 + x].fxd2 != -1)
                    break;
            }

            font->dynamic[y * 16 + x].width =
                (font->dynamic[y * 16 + x].fxd1 == -1
                     ? 0
                     : font->dynamic[y * 16 + x].fxd2 + 1
                           - font->dynamic[y * 16 + x].fxd1);
            if (font->dynamic[y * 16 + x].width) {
                font->dynamic[y * 16 + x].fxd1 =
                    (font->dynamic[y * 16 + x].fxd1)
                    / ((float) (req_width - 1));
                font->dynamic[y * 16 + x].fxd2 =
                    (font->dynamic[y * 16 + x].fxd2)
                    / ((float) (req_width - 1));
            }
            ...
            if (noegnud_options_render_opengl_mipmapping->value) {
                ...
                glTexImage2D(GL_TEXTURE_2D, 0, 4, req_width, req_height, 0,
...
noegnud_fonts_tfont *
noegnud_fonts_loadfont(const char *name)
{
    noegnud_fonts_tfont *font;
    char *szHomedir;
    noegnud_tcollection *font_placement;

    if ((font = noegnud_collection_data(noegnud_fonts_fontsloaded, name))
        && font->loaded) {
        font->loaded++;
...
void
noegnud_fonts_printchar_scaled(noegnud_fonts_tfont *font, unsigned char ch,
    ...
    while (*ch) {
        while (*ch == NOEGNUD_TEXTCOLOUR_CONTROL) {
            ...
            case NOEGNUD_TEXTCOLOUR_CODE_RGBA:
                ...
                /*
                  default:
                  printf("[ERROR] internal font colour
                  syntax\n\t\t\"%s\"\n",string);
                */
            }
        }
...
noegnud_width_dynamic(noegnud_fonts_tfont *font, char *string)
{
    ...
    ch = string;
    while (*ch) {
        width_count += (font->dynamic[(int) *ch].width
                            ? font->dynamic[(int) *ch].width + 1
                            : 4);
        ch++;
    }
    return width_count;
                            ...
                            char *string)
{
    ...
    while (*ch) {
        ...
        noegnud_fonts_printchar_dynamic(font, (char) (*ch));
        glTranslated(font->dynamic[(int) *ch].width
                         ? font->dynamic[(int) *ch].width + 1
                         : 4,
                     0, 0);
        ch++;
...
void
noegnud_fonts_deletefont(noegnud_fonts_tfont **font_)
{
    noegnud_fonts_tfont *font;
    ...
    if (!font->loaded) {
        ...
        printf("* Unloading font (cached:%d): %s\n", font->loaded,
               font->imagefilename);
}
