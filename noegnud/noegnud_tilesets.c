...
void
noegnud_tilesets_clearloaded(ts_tileset *tileset)
{
    int c;
    ...
    for (c = 0; c < MAX_TILES; c++)
        if (tileset->loaded[c]) {
            if (tileset->tiles[c]
                != noegnud_image_tilenotfound->block[0][0]->image)
                glDeleteTextures(1, &tileset->tiles[c]);
            tileset->loaded[c] = FALSE;
        }
}

ts_tileset *
noegnud_tilesets_loadtileset(const char *name)
{
    char infname[1024];
    char *szHomedir;
    ...
    /*
            loading->glwidth =
       ((float)loading->width/32)*noegnud_options_render_multiplier->value;
            loading->glheight =
       ((float)loading->height/32)*noegnud_options_render_multiplier->value;
    */
    loading->glwidth =
        ((((float) loading->width / (float) loading->height) * 64.0) / 32)
        * noegnud_options_render_multiplier->value;
    loading->glheight =
        (64.0 / 32) * noegnud_options_render_multiplier->value;

    if (!loading->xdeltax)
        loading->xdeltax = loading->width;
    if (!loading->ydeltay)
        loading->ydeltay = loading->height;
    loading->xdeltax =
        ((loading->xdeltax / 32) * noegnud_options_render_multiplier->value)
        * (64.0 / (float) loading->height);
    loading->xdeltay =
        ((loading->xdeltay / 32) * noegnud_options_render_multiplier->value)
        * (64.0 / (float) loading->height);
    loading->ydeltax =
        ((loading->ydeltax / 32) * noegnud_options_render_multiplier->value)
        * (64.0 / (float) loading->height);
    loading->ydeltay =
        ((loading->ydeltay / 32) * noegnud_options_render_multiplier->value)
        * (64.0 / (float) loading->height);
    ...
    for (c = 0; c < MAX_TILES; c++)
        loading->loaded[c] = FALSE;
    /*
            if ( !noegnud_tilesetload_images_from( loading, name, (char
       *)loading->location) ) { noegnud_mem_free(loading); loading=NULL;
            }
    */
    return loading;
}
...
void
noegnud_tilesets_unloadtileset(ts_tileset *tileset)
{
...
int
noegnud_tilesetload_image_from(ts_tileset *tileset, int tilenum)
{
    ...
    if (!(tile_reference =
              noegnud_collection_structure(noegnud_reference, sztilenum)))
        return 0;

    if (!tileset->loaded[tilenum]) {
        filename = noegnud_mem_malloc(1024);
        sprintf(filename, "%s/%s.png", location,
                (char *) tile_reference->data);
