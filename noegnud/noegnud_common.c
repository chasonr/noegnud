...
{
    /* this is _SO_ ugly :/ i really need to figure out a "cleaner" way ......
...
char *
noegnud_common_str_replace(const char *haystack, const char *needle,
                           const char *camel)
{
    char *pos;
    char *result;
    ...
    while ((pos = strstr(result, needle))) {
        ...
        resultn = noegnud_mem_malloc(strlen(result) - strlen(needle)
                                     + strlen(camel) + 1);
...
int
noegnud_common_file_exists(const char *filename)
{
    FILE *f;
